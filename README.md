...
### Soon to be added
- Product categories that have been added as custom fields in the ERP automatically sync and map to Medusa/website categories. Product categories are not natively offered by this ERP.
...
- I am begrudgingly using their endpoint that takes raw SQL as a query parameter for product/inventory sync to circumvent that. The queries are hardcoded on the website's backend & no user input which minimizes risk
